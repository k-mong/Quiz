...
public class Aes256Util {   // AES 뒤에 붙은 숫자는 키의 길이 의ㅣ미 외부에 노출되어서는 안됨. 256비트(32바이트)
    ...
    private static String KEY = "quizProgramIsKeyJustKey";  // 암호화 키
    ...
    public static String encrypt(String text){  // 암호화되지 않은 데이터를 암호화 시킴
        try{
            ...
            IvParameterSpec ivParameterSpec = new IvParameterSpec(IV.getBytes());   // ? 이거 모르겠음 초기화된 벡터를 반환
